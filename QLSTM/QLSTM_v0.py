...
class VQC(nn.Module):
	def __init__(self, vqc_depth, n_qubits, n_class):
		...
		# self.dev = qml.device("lightning.qubit", wires=n_qubits, max_workers=os.cpu_count()-2)  # ! 
	...
	def forward(self, X):
		...
		# return y_preds
		return self.VQC(X, self.weights, self.n_class)
...
def main():
	...
	# dtype = torch.DoubleTensor  # ! modify
	...
	input_data = torch.randn(batch_size, seq_length, input_size).float()
	...
	for i in range(100):
		...
		print(f"Epoch {i+1} 執行時間: {epoch_duration:.2f} 秒")
		print("-" * 30)

	total_end_time = time.time()
	total_duration = total_end_time - total_start_time
	print(f"\n程式碼總執行時間: {total_duration:.2f} 秒 ({total_duration/60:.2f} 分鐘)")
	return
